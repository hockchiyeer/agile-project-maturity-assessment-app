Feature: Custom suite persistence and export coverage
  ...
  Scenario Outline: Persist and export state with the source-style custom suite on <viewport>
    ...
    Then I verify "save status" should contain "JSON not linked" on "Agile_Maturity_App"
    When I click on the "link json button" on "Agile_Maturity_App"
    Then I verify "save status" should contain "Saved to" on "Agile_Maturity_App"
    When I set question "Technical excellence" field "score" to "4" in the custom suite
    ...
    Then I verify "save status" should contain "Downloaded JSON snapshot" on "Agile_Maturity_App"
    And I verify downloaded JSON contains question "Technical excellence" with score "4" in the custom suite
    ...
    And I verify "save status" should contain "Loaded saved-assessment.json" on "Agile_Maturity_App"
    ...
    And I verify "save status" should contain "PDF exported successfully" on "Agile_Maturity_App"
    ...
    And I verify "save status" should contain "PPT exported successfully" on "Agile_Maturity_App"
